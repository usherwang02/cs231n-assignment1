...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    correct_class_score = np.expand_dims(scores[range(N), y], axis=1)
    sum = np.sum(np.exp(scores), axis=1)
    loss = (np.sum(np.log(sum)) - np.sum(correct_class_score)) / N + reg * (np.sum(W1 * W1) + np.sum(W2 * W2))
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    ...
    item[range(N), y] -= 1
    dscore = item / N
    dW2 = hidden_layer.T.dot(dscore) + 2 *reg * W2
    db2 = np.sum(dscore, axis=0)
    dhidden = dscore.dot(W2.T)
    dhidden[hidden_layer <= 0] = 0
    dW1 = np.dot(X.T, dhidden) + 2 * reg * W1
